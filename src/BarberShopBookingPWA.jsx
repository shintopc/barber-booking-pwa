...
export default function BarberShopBookingPWA() {
  ...
  const [slotMinutes] = useState(60);
  const [seatsPerSlot] = useState(5);
  ...
  function clearMessageAfter(ms = 3000) {
  ...
  function handleBook(slotLabel) {
    ...
    // Customer confirmation (on-screen only)
    setMessage({ 
      type: "success", 
      text: `✅ Booked ${slotLabel} on ${selectedDate}. Show this message at the shop.` 
    });
    clearMessageAfter();

    // WhatsApp notification to owner only
    const ownerNumber = "9961583051"; // Your number without +
    const ownerMsg = encodeURIComponent(
      ...
      `• Date: ${selectedDate}\n` +
      `• Time: ${slotLabel}\n\n` +
      `Total today: ${Object.keys(todaysBookings).length + 1} bookings`
    );
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(#fff7ed,#fff1f2)" }}>
      <div style={{ 
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
          ...
          >
            {ownerMode ? "Customer View" : "Owner Settings"}
          </button>
        </div>
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        ...
                        padding: "8px 10px", 
                        borderRadius: 8, 
                        background: c >= seatsPerSlot ? "#fed7d7" : "#ecfdf5", 
                        marginBottom: 8 
                      }}
                    >
              ...
              {slots.map((s) => {
                ...
                return (
                  <button 
                    key={s.label} 
                    disabled={!free} 
                    onClick={() => handleBook(s.label)} 
                    style={{ 
                      padding: 12, 
                      borderRadius: 10, 
                      fontWeight: 700, 
                      boxShadow: free ? "0 6px 18px rgba(16,185,129,0.18)" : "none", 
                      border: "none", 
                      background: free ? "#34d399" : "#fca5a5", 
                      color: free ? "white" : "#7f1d1d" 
                    }}
                  >
                    <div>{s.label}</div>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <div>
            {Object.keys(todaysBookings).length === 0 && (
                  ...
                  <div>
                    ...
                    {todaysBookings[slot].map((b) => (
                      <div key={b.id} style={{ fontSize: 13, marginTop: 6 }}>
                        {b.name} — {b.whatsapp} 
                        <button 
                          onClick={() => cancelBooking(slot, b.id)} 
                          style={{ 
                            marginLeft: 8, 
                            color: "#dc2626", 
